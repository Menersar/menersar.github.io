...
async function run(text) {
    ...
    for (i = 0; i < 5; i++) {
        ...
        textNumber[i] = jsonparsed[text[text.length - i]]
    }
    ...
    // sequence = [1, 1, 1, 1, 1]
    const tensor = tf.tensor2d(textNumber, [1, 5]);

    const [words] = tf.tidy(() => {
        const pwords = model.predict(tensor);
    ...
    descPredictions = [...words];

    descPredictions.sort((a, b) => b - a);

    for (i = 0; i < 5; i++) {
        const index = words.indexOf(descPredictions[i]);
        console.log(descPredictions)
        predictedWords.push(
            Object.keys(jsonparsed).find(
                (key) => jsonparsed[key] === index
            )
        );
    }

    console.log("test:" + predictedWords)
